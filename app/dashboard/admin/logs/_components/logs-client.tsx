...
export default function LogsClient({ initialData }: { initialData: any }) {
    const { logs: mockLogs } = initialData;
    ...
    const filteredLogs = mockLogs.filter((log: any) => {
    ...
    const getLogStats = () => {
        ...
        const errors = mockLogs.filter((l: any) => l.status === 'failed' || l.status === 'error').length;
        const warnings = mockLogs.filter((l: any) => l.status === 'warning').length;
        const today = mockLogs.filter((l: any) => {
