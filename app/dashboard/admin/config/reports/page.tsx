...
export default function ReportTemplatesPage() {
  ...
  const { reportTemplates, addReportTemplate, updateReportTemplate, deleteReportTemplate } = useAppStore();
  ...
  const handleAddEditTemplate = () => {
    ...
    if (editingTemplate) {
      updateReportTemplate(editingTemplate.id, { ...templateData, updatedAt: new Date().toISOString() });
  ...
  const handleAddSection = () => {
    ...
    setNewSection({
      ...
      dataFields: [],
      chartType: 'bar',
      position: templateData.sections.length + 1,
    });
  ...
  const handlePreviewTemplate = (template: ReportTemplate) => {
    // In a real application, this would render a preview of the report
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage templates for various downloadable reports (e.g., student transcripts, department summaries).
          </p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[700px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Define the structure and content for your reports.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Select
                  ...
                  onValueChange={(value) => setTemplateData({ ...templateData, category: value as ReportTemplate['category'] })}
                >
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="template-layout" className="text-right">
                  Layout
                </Label>
                <Select
                  value={templateData.layout}
                  onValueChange={(value) => setTemplateData({ ...templateData, layout: value as ReportTemplate['layout'] })}
                >
                  <SelectTrigger id="template-layout" className="col-span-3">
              ...
              <div className="col-span-4 border-t pt-4 mt-4 space-y-3">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setNewSection({ ...newSection, type: value as ReportSection['type'] })}
                  >
                    <SelectTrigger id="section-type" className="col-span-3">
                ...
                {newSection.type === 'chart' && (
                  <div className="grid grid-cols-4 items-center gap-4">
                    <Label htmlFor="section-chartType" className="text-right">
                      Chart Type
                    </Label>
                    <Select
                      value={newSection.chartType}
                      onValueChange={(value) => setNewSection({ ...newSection, chartType: value as ReportSection['chartType'] })}
                    >
                      <SelectTrigger id="section-chartType" className="col-span-3">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="section-dataFields" className="text-right">
                    Data Fields
                  </Label>
                  <Input
                    id="section-dataFields"
                    value={newSection.dataFields.join(', ')}
                    onChange={(e) => setNewSection({ ...newSection, dataFields: e.target.value.split(',').map(f => f.trim()).filter(f => f) })}
                    className="col-span-3"
                    placeholder="e.g., studentGrades, modulePerformance"
                  />
                </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Existing Report Templates</CardTitle>
          <CardDescription>
            Configure, edit, or delete report templates.
          </CardDescription>
        </CardHeader>
        <CardContent>
