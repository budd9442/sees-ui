...
export default function AdvisorStudentsPage() {
  ...
  const atRiskStudents = myStudents.filter(s => s.currentGPA < 2.5).length;
  const excellentStudents = myStudents.filter(s => s.currentGPA >= 3.7).length;
  ...
  const getGPATrend = (student: Student) => {
  ...
  const handleSendMessage = (studentId: string) => {
    // Navigate to messages with student pre-selected
    router.push(`/dashboard/advisor/messages?student=${studentId}`);
  };

  const handleScheduleMeeting = (studentId: string) => {
    // Navigate to meetings with student pre-selected
    router.push(`/dashboard/advisor/meetings?student=${studentId}`);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage and monitor your advisees' academic progress
          </p>
        </div>
      ...
      <Tabs defaultValue="all" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="at-risk">At Risk ({myStudents.filter(s => s.currentGPA < 2.5).length})</TabsTrigger>
          <TabsTrigger value="excellent">Excellent ({myStudents.filter(s => s.currentGPA >= 3.7).length})</TabsTrigger>
        ...
        <TabsContent value="all" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Complete list of your advisees
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            ...
                            <Button 
                              variant="outline" 
                              size="sm"
                              onClick={() => handleScheduleMeeting(student.studentId)}
                            >
        ...
        <TabsContent value="at-risk" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Students requiring immediate attention (GPA &lt; 2.5)
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {myStudents.filter(s => s.currentGPA < 2.5).map((student) => (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          GPA: {student.currentGPA.toFixed(2)} • {student.studentId} • {student.specialization}
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" variant="default" onClick={() => handleScheduleMeeting(student.studentId)}>
                        Schedule Meeting
                      </Button>
                    </div>
                  </div>
        ...
        <TabsContent value="excellent" className="space-y-4">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {myStudents.filter(s => s.currentGPA >= 3.7).map((student) => (
                  <div key={student.id} className="flex items-center justify-between p-4 rounded-lg border border-green-200 bg-green-50">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => handleSendMessage(student.studentId)}>
                        Send Message
                      </Button>
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          ...
                          {index === 2 && 'Scheduled advising meeting'}
                          {index === 3 && 'Submitted pathway selection'}
                          {index === 4 && 'Viewed grade report'}
                        </p>
                      </div>
                    </div>
