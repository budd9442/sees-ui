...
import type { Student, Grade, Module } from '@/types';
...
export default function ReportsPage() {
  ...
  const reportTemplates = [
    {
      ...
      description: 'Comprehensive overview of department performance metrics',
      icon: BarChart3,
    },
    {
      id: 'pathway-demand',
      name: 'Pathway Demand Analysis',
      description: 'Analysis of pathway selection trends and capacity utilization',
      icon: TrendingUp,
    },
    {
      id: 'academic-class-distribution',
      name: 'Academic Class Distribution',
      description: 'Distribution of students across academic classes',
      icon: GraduationCap,
    },
    {
      id: 'module-performance',
      name: 'Module Performance Comparison',
      description: 'Performance comparison across different modules',
      icon: BookOpen,
    },
    {
      id: 'retention-graduation',
      name: 'Retention & Graduation Rates',
      description: 'Student retention and graduation rate analysis',
      icon: Award,
    },
    {
      id: 'at-risk-students',
  ...
  const getDepartmentStats = () => {
    ...
    const allGrades = grades.filter(g => g.isReleased);
    const totalPoints = allGrades.reduce((sum, grade) => sum + (grade.points * grade.credits), 0);
    ...
    const passRate = allGrades.length > 0 
      ? (allGrades.filter(g => g.points >= 2.0).length / allGrades.length) * 100 
      : 0;
    ...
    const atRiskStudents = students.filter(student => {
      ...
      const studentCredits = studentGrades.reduce((sum, grade) => sum + grade.credits, 0);
      const studentGPA = studentCredits > 0 ? studentPoints / studentCredits : 0;
      return studentGPA < 2.5 || studentGrades.some(g => g.points < 2.0);
    }).length;
  ...
  const getModulePerformanceData = () => {
    return modules.map(module => {
      ...
      return {
        ...
        avgGrade,
        passRate,
        enrolledStudents: moduleGrades.length,
      };
    });
  };
  ...
  const getAtRiskStudentsData = () => {
    return students.filter(student => {
      ...
      const studentCredits = studentGrades.reduce((sum, grade) => sum + grade.credits, 0);
      const studentGPA = studentCredits > 0 ? studentPoints / studentCredits : 0;
      const interventionsCount = interventions.filter(i => i.studentId === student.id).length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Generate comprehensive reports for department analysis
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{getDepartmentStats().totalStudents}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{getDepartmentStats().avgGPA.toFixed(2)}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{getDepartmentStats().passRate.toFixed(1)}%</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{getDepartmentStats().atRiskStudents}</div>
            <p className="text-xs text-muted-foreground">Students needing support</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="templates" className="space-y-4">
        ...
        <TabsContent value="templates" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Select a template and configure parameters to generate a report
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setFilters({ ...filters, pathway: value })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
        ...
        <TabsContent value="analytics" className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle>Pathway Demand</CardTitle>
                <CardDescription>
                  Student distribution across pathways
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={getPathwayDemandData()}>
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      ...
                      dataKey="students"
                    >
                      {getAcademicClassData().map((entry, index) => (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Average grades and pass rates across modules
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={getModulePerformanceData()}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="code" />
                  <YAxis yAxisId="left" />
                  <YAxis yAxisId="right" orientation="right" />
                  <Tooltip />
                  <Bar yAxisId="left" dataKey="avgGrade" fill="#8884d8" name="Avg Grade" />
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold">
                    {reportTemplates.find(t => t.id === generatedReport.template)?.name}
                  </h3>
                ...
                <div className="space-y-4">
                  <div className="grid gap-4 md:grid-cols-4">
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">{generatedReport.departmentStats.totalStudents}</div>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-3">
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">{generatedReport.departmentStats.avgGPA.toFixed(2)}</div>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-3">
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">{generatedReport.departmentStats.passRate.toFixed(1)}%</div>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-3">
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">{generatedReport.departmentStats.atRiskStudents}</div>
                      </CardContent>
                    </Card>
                  </div>
                </div>
