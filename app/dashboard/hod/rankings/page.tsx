...
export default function RankingsPage() {
  ...
  // Calculate rankings with weighted tiebreaker
  const calculateRankings = () => {
    const rankings: RankingEntry[] = students.map(student => {
      ...
      // Calculate other metrics for tiebreaking
      const creditsEarned = studentGrades.filter(g => g.points >= 2.0).reduce((sum, g) => sum + g.credits, 0);
      const attendance = Math.floor(Math.random() * 20) + 80; // Mock attendance
      const participation = Math.floor(Math.random() * 20) + 80; // Mock participation
      
      // Calculate weighted score for tiebreaking
      const weightedScore = 
    ...
    // Sort by GPA first, then by weighted score for tiebreaking
    rankings.sort((a, b) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Academic rankings with weighted tiebreaker logic
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setShowTiebreakDialog(true)}>
            <Target className="mr-2 h-4 w-4" />
            Configure Tiebreakers
          </Button>
          <Button variant="outline" onClick={() => exportRankings('pdf')}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configure batch, semester, and filters for rankings
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">In ranking</p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {filteredRankings.length > 0 
                ? (filteredRankings.reduce((sum, r) => sum + r.gpa, 0) / filteredRankings.length).toFixed(2)
                : '0.00'
              }
            </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-medium">Ties Detected</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{ties.length}</div>
            <p className="text-xs text-muted-foreground">GPA groups</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {filteredRankings.filter(r => r.gpa >= 3.5).length}
            </div>
      ...
      <Tabs defaultValue="rankings" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="ties">Tie Analysis</TabsTrigger>
        ...
        <TabsContent value="rankings" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Rankings based on GPA with weighted tiebreaker logic
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Tiebreak Score</TableHead>
                ...
                <TableBody>
                  {filteredRankings.map((ranking) => {
                    ...
                    const isTied = ties.some(tie => tie.students.some(s => s.id === ranking.id));
        ...
        <TabsContent value="ties" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Tie Analysis</CardTitle>
              <CardDescription>
                Analysis of tied GPAs and tiebreaker resolution
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {ties.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p>No ties detected in current rankings</p>
                  </div>
                ) : (
                  ties.map((tie, index) => (
                    <div key={index} className="p-4 rounded-lg border">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h4 className="font-semibold">Tied GPA: {tie.gpa.toFixed(2)}</h4>
                        <Badge variant="outline">{tie.students.length} students</Badge>
                      </div>
                      <div className="space-y-2">
                        {tie.students.map((student) => (
                      ...
                      <div className="mt-3 text-sm text-muted-foreground">
                        <strong>Tiebreaker factors:</strong> GPA ({tiebreakWeights.gpa}), Credits ({tiebreakWeights.credits}), 
                        Attendance ({tiebreakWeights.attendance}), Participation ({tiebreakWeights.participation})
                      </div>
                    </div>
                  ))
                )}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      data={getAcademicClassDistribution()}
                      cx="50%"
                      cy="50%"
                      labelLine={false}
                      ...
                      dataKey="students"
                    >
                      {getAcademicClassDistribution().map((entry, index) => (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Distribution of students by pathway
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={getPathwayDistribution()}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {getAcademicClassDistribution().map((item) => (
                  <div key={item.academicClass} className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span>{item.academicClass}</span>
                      <span>{item.students} students ({item.percentage.toFixed(1)}%)</span>
                    </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-yellow-600">
                        {filteredRankings.filter(r => r.gpa >= 3.7).length}
                      </div>
                      <p className="text-xs text-muted-foreground">GPA ≥ 3.7</p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-blue-600">
                        {filteredRankings.filter(r => r.gpa >= 3.0 && r.gpa < 3.7).length}
                      </div>
                      <p className="text-xs text-muted-foreground">GPA 3.0-3.69</p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-green-600">
                        {Math.min(10, filteredRankings.length)}
                      </div>
                ...
                <div className="space-y-3">
                  <h4 className="font-semibold">Certificate Templates</h4>
      ...
      <Dialog open={showTiebreakDialog} onOpenChange={setShowTiebreakDialog}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Configure Tiebreaker Weights</DialogTitle>
            <DialogDescription>
              Set the weights for tiebreaker factors when GPAs are tied
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0"
                  max="1"
                  value={tiebreakWeights.gpa}
                  onChange={(e) => setTiebreakWeights({ ...tiebreakWeights, gpa: parseFloat(e.target.value) })}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0"
                  max="1"
                  value={tiebreakWeights.credits}
                  onChange={(e) => setTiebreakWeights({ ...tiebreakWeights, credits: parseFloat(e.target.value) })}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0"
                  max="1"
                  value={tiebreakWeights.attendance}
                  onChange={(e) => setTiebreakWeights({ ...tiebreakWeights, attendance: parseFloat(e.target.value) })}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="0"
                  max="1"
                  value={tiebreakWeights.participation}
                  onChange={(e) => setTiebreakWeights({ ...tiebreakWeights, participation: parseFloat(e.target.value) })}
                />
              </div>
            </div>
            ...
            <div className="p-3 rounded-lg bg-muted">
              <div className="text-sm font-medium">Total Weight: {
                (tiebreakWeights.gpa + tiebreakWeights.credits + tiebreakWeights.attendance + tiebreakWeights.participation).toFixed(1)
              }</div>
              {(tiebreakWeights.gpa + tiebreakWeights.credits + tiebreakWeights.attendance + tiebreakWeights.participation) !== 1.0 && (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowTiebreakDialog(false)}>
              Cancel
            </Button>
            <Button 
              onClick={() => {
                setShowTiebreakDialog(false);
                toast.success('Tiebreaker weights updated successfully!');
              }}
              disabled={(tiebreakWeights.gpa + tiebreakWeights.credits + tiebreakWeights.attendance + tiebreakWeights.participation) !== 1.0}
            >
              <Target className="mr-2 h-4 w-4" />
              Update Weights
            </Button>
