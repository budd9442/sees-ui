...
`;
    ...
    static async generatePersonalizedFeedback(context: Record<string, unknown>) {
        ...
        const fallback = {
            overallAssessment: "Your academic performance is stable and you are making progress toward your degree requirements.",
            strengths: ["Steady completion of released modules", "Clear academic continuity across semesters"],
            riskAreas: ["Monitor upcoming assessment-heavy modules to avoid sudden GPA dips."],
            goalGuidance: ["Keep your active goals measurable and tied to module outcomes each semester."],
            recommendedActions: [
                "Review feedback from your latest released modules and identify one improvement area.",
                "Allocate weekly focused study blocks for weaker modules.",
            ],
            next30DayPlan: [
                "Week 1: audit current module workload and deadlines.",
                "Week 2: complete two targeted revision sessions in lowest-confidence topics.",
                "Week 3: validate progress with a self-test and refine plan.",
                "Week 4: prepare advisor check-in with evidence of progress.",
            ],
            advisorEscalationSignals: [
                "Multiple failing/near-failing results in released grades",
                "Noticeable sustained GPA decline over consecutive periods",
            ],
            confidenceNotes: {
                modelUsed: false,
                summary: "Fallback generated because AI response was unavailable.",
            },
        };
        ...
        try {
            const prompt = `
You are an academic advisor AI for university students.
Analyze the student context JSON and return STRICT JSON only.
...
Return this exact shape:
{
  ...
  "advisorEscalationSignals": ["string"],
  "confidenceNotes": {
    "modelUsed": true,
    "summary": "string"
  }
...
`;
            ...
            return {
                ...
                confidenceNotes: {
                    modelUsed: true,
                    summary: parsed?.confidenceNotes?.summary || "Generated by Gemini from full student context.",
                },
            };
        } catch {
            return fallback;
